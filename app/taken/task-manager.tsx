...
export default function WeekBoard({
  ...
  const sensors = useSensors(
    // Muis (desktop): heel kaartje blijft direct sleepbaar, zoals altijd
    useSensor(MouseSensor, { activationConstraint: { distance: 6 } }),
    // Touch (mobiel): alleen actief via de aparte sleepknop op het kaartje
  ...
  function Kaart({ b, overlay }: { b: Block; overlay?: boolean }) {
    ...
    // dnd-kit levert per geregistreerde sensor een eigen event-handler in
    // "listeners" (bv. onMouseDown voor MouseSensor, onTouchStart voor
    // TouchSensor). Door ze te splitsen kan de muis het hele kaartje
    // gebruiken (desktop, zoals altijd), terwijl touch alleen via de
    // aparte sleepknop werkt (mobiel) — zo blijft swipen over het
    // kaartje mogelijk zonder dat het conflicteert met slepen.
    const alleListeners = (overlay ? {} : listeners ?? {}) as Record<string, ((event: any) => void) | undefined>
    const muisSleep = { onMouseDown: alleListeners.onMouseDown }
    const toetsSleep = { onTouchStart: alleListeners.onTouchStart }
    ...
    return (
      ...
      >
        <div
          {...(overlay ? {} : attributes)}
          {...(overlay ? {} : muisSleep)}
          className={overlay ? '' : 'cursor-grab active:cursor-grabbing'}
        >
        ...
        {!overlay && (
          <div className="mt-2 flex items-center gap-1">
            ...
            <button
              ...
              title="Verslepen (mobiel)"
              aria-label="Verslepen"
              className="ctl tap touch-none rounded px-2 py-1 text-base font-bold sm:hidden"
            >
              &#10021;
            </button>
            <button
  ...
  return (
    <div className="mt-2 overflow-x-hidden">
      <div className="sticky top-0 z-30 -mt-1 border-b border-[var(--brd)] bg-[var(--background)]/95 pb-3 pt-1 backdrop-blur sm:static sm:border-0 sm:bg-transparent sm:pb-0 sm:pt-0">
        ...
        {view === 'week' ? (
          ...
          <div className="mt-2 flex w-full items-center gap-2 sm:mt-3">
            <button type="button" onClick={() => gaDag(-1)} className="ctl tap shrink-0 rounded-lg px-4 py-2 text-sm">←</button>
            <div className="surface-2 flex min-w-0 flex-1 items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-medium">
              <span className="truncate">{DAGEN[dagIndex]} {formatDag(dagen[dagIndex])}</span>
              {huidigeISO === vandaag && (
                <span className="shrink-0 rounded bg-blue-600 px-1.5 py-0.5 text-xs text-white">vandaag</span>
              )}
            </div>
            <button type="button" onClick={() => gaDag(1)} className="ctl tap shrink-0 rounded-lg px-4 py-2 text-sm">→</button>
          </div>
